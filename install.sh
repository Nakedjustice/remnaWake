...
write_override_file() {
  local need_bot_detail="no" need_autoupdate_network="no" need_remnawave_network="no" tmp
  if [ "$HOST_PROXY_ENABLED" = "yes" ] || [ "$ALONGSIDE_REMNAWAVE" = "yes" ] || [ -n "$WATCHTOWER_URL" ]; then
    need_bot_detail="yes"
  fi
...
doctor_mode() {
  local failures=0 warnings=0 compose value base_image autoupdate_image web_port mode
  ...
  if [ -f "$COMPOSE_FILE" ]; then
    base_image="$(sed -n 's/^[[:space:]]*image:[[:space:]]*//p' "$COMPOSE_FILE" | head -n 1 | tr -d '"')"
    case "$base_image" in
    ...
    if [ -n "$base_image" ] && [ "$autoupdate_image" != "$base_image" ]; then
      check_warn "AUTOUPDATE_IMAGE ($autoupdate_image) differs from compose image ($base_image)"
    else
      check_ok "AUTOUPDATE_IMAGE matches compose image"
    fi
  fi
