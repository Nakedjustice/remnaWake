...
func TestSendAdminMenu(t *testing.T) {
	...
	if m.Keyboard == nil || len(m.Keyboard.InlineKeyboard) != 17 {
		t.Fatalf("expected 17 keyboard rows, got %+v", m.Keyboard)
	}
	callbackData := func(row int) string {
		return m.Keyboard.InlineKeyboard[row][0].CallbackData
	}
	for i, want := range []string{"adm:pending", "adm:stats", "adm:tariffs", "adm:addtariff", "adm:del_list", "adm:traffic_ext", "adm:req", "adm:gifts", "adm:setreq", "adm:shot_toggle", "adm:squad", "adm:user", "adm:treset", "adm:trial", "adm:referral", "adm:upd", "adm:bcast"} {
