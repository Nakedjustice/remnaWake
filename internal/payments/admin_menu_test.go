...
func TestSendAdminMenu(t *testing.T) {
	...
	if m.Keyboard == nil || len(m.Keyboard.InlineKeyboard) != 9 {
		t.Fatalf("expected 9 keyboard rows, got %+v", m.Keyboard)
	}
	callbackData := func(row int) string {
		return m.Keyboard.InlineKeyboard[row][0].CallbackData
	}
	for i, want := range []string{"adm:stats", "adm:tariffs", "adm:addtariff", "adm:del_list", "adm:req", "adm:gifts", "adm:setreq", "adm:squad", "adm:bcast"} {
